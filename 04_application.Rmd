...
```{r initial_inspection_of_pisa, echo=1, eval=T, cache=TRUE, fig.width=6, dev.args = list(bg = 'transparent')}
...
p = ggpairs(pisa[,-c(1,3:5)], 
        lower=list(continuous=wrap(better_smooth,  ptalpha=.25, ptcol='#ff5500', ptsize=1,
                                   linecol='#03b3ff', method='loess', se=F, lwd=.5)), 
        diag = list(continuous=wrap('densityDiag', color='gray50', lwd=.5)),
        upper=list(continuous=wrap(better_corr)),
        axisLabels="none") + 
  theme_trueMinimal() +
...
Now we examine the effects of interest visually via component plots[^componentplot]. The following uses the <span class="func">visreg</span> function.
