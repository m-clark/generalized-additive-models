...
```{r initial_inspection_of_pisa_visual, echo=FALSE, fig.width=6}
...
```{r csbyhand, echo=FALSE, out.width='40%', fig.asp=.5}
rk <- function(x,z) {
...
```{r plot_mod_gam1, echo=2, eval=c(1,3:5), out.width='75%', fig.asp=.75}
plot_gam(mod_gam1) +
  ylab('s(Income, 6.9)') +
...
Note that this will only plot the smooth terms in the model<span class="marginnote">A more visually appealing alternative can be found with the <span class="pack">visreg</span> package, which will plot all effects, and by default be nicer looking and easier to customize. You can simply feed the model to the <span class="func">visreg</span> function. It also has 2 and 3d capabilities like the corresponding <span class="pack">mgcv</span> functions.  In most places, I'm using my own function from <span class="pack">visibly</span>.</span>, which you can get from [GitHub](https://m-clark.github.io/visibly/). The intervals are Bayesian credible intervals based on the posterior predictive distribution. In this single predictor case, one can also revisit the previous graph.  
...
```{r plot_mod_gam2_responseB, echo=F, out.width='40%', fig.asp=.5}
ggplotly(width=600) %>% 
  config(displayModeBar=F) %>% 
  layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
```
...
```
```{r contourheat, echo=FALSE, cache=FALSE}
...
```{r mod_gam3_plot, fig.show='hide', echo=F, cache=FALSE}
