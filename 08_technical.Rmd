...
```{r csbsPlot, echo=FALSE, out.width='40%', fig.asp=.5}
...
```{r csbsScaledPlotData, echo=c(1:4,6), out.width='45%', fig.asp=.5}
...
```{r csbsScaledPlot, echo=F, out.width='45%', fig.asp=.5}
...
```{r csbsFitPlotDegree1, echo=FALSE, out.width='40%', fig.asp=.5}
...
```{r csFitPlotDegree3, echo=FALSE, out.width='40%', fig.asp=.5}
l = 3
bs = sapply(1:length(knots), function(k) ifelse(x >= knots[k], (x-knots[k])^l, 0))
...
```{r csFitPlotvsGAM, echo=FALSE, out.width='45%', fig.asp=.5}
library(mgcv)
...
```{r bSpline, echo=FALSE, out.width='40%', fig.asp=.5}
...
```{r differentNKnots, echo=FALSE, out.width='45%', fig.asp=.5}
library(dplyr)
l = 3
nk = c(3, 6, 9, 15)
bs = lmCS = list()
...
```{r termplot_visreg, echo=-9, eval=T}
pred_data = pisa %>% 
  select(Edu, Health) %>%
...
We can easily extract this information using the <span class="pack">schoenberg</span> package[^schoenberg], specifically with the <span class="func">fderiv</span> function.  This allows one to get the estimated slope at various points, either by specifying them with the `newdata` argument or specifying the number of values desired along the range of the the model covariate, as below.

```{r fderiv, echo=1:3, out.width='40%', fig.asp=.5}
library(schoenberg)

fd_inc = confint(fderiv(mod_gam1, n = 500))

fd_inc = fd_inc %>% 
  mutate(Income = seq(min(mod_gam1$model$Income), max(mod_gam1$model$Income), length.out = 500)) %>% 
  rename(Estimate = est)

plot_ly(fd_inc) %>% 
  add_lines(x=~Income,  y=0, color=I('gray92'), showlegend=F) %>% 
  add_lines(x=~Income, y=~Estimate, showlegend=F) %>% 
  add_markers(x=~Income, y=~Estimate, 
              showlegend=F, 
              color = I('#00aaff'),
              size=I(10),
...
While we are getting something we can get a sense by looking at the standard effect plot, we can get more precise with this approach if desired. Here we see that the initial flattening a little after .6 on Income, and both the peak positive slope and sharpest negative slope (blue dots).  Depending on the modeling context, one may see patterns of theoretical importance or what theory would predict.  In other cases it just gives you more to talk about.
...
[^schoenberg]: Fairly new and still being developed as of this writing, it is [available on GitHub](https://github.com/gavinsimpson/schoenberg).