...
# sapply(
#   1:3, 
#   function(k) ifelse(x2 >= knots[k], (x2 - knots[k]) ^ l, 0)
# )
```
...
```{r csbsFitPlotDegree1, echo=FALSE, cache.rebuild=T, fig.asp=.5}
...
fd_inc %>% 
  ...
  geom_point(
    size = 3,
    alpha = 1,
    data = fd_inc %>% filter(Estimate == max(Estimate) |
