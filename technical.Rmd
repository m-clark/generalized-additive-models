...
$$\epsilon \sim N(0,.3^2)$$
</span>.Let's see it in action. Here our polynomial spline will be done with degree $l$ equal to 1, which means that we are just fitting a linear regression between knots This uses the data we employed for demonstration [before][A more complex relationship].
...
```{r termplot_visreg, echo=-9, out.width='75%'}
pred_data = pisa %>% 
  select(Edu, Health) %>%
