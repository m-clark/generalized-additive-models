...
ggplotly() %>% layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', width='100%')
```
...
csplot = ggplot(aes(x=x, y=wear), data=d) +
  ...
  theme(plot.background=element_blank(),
        panel.background=element_blank())
ggplotly() %>% layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
```
...
```{r echo=FALSE}
plot_ly(x=sort(Income), y=fitted(lmMod)[order(Income)], name='unpenalized', width='100%') %>% 
