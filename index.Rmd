...
link-citations: yes
description: "Generalized Additive Models"    # leaving this blank will actually screw things up in the same way as the url issue; even commenting will
...
knitr::opts_chunk$set(cache.rebuild=F, cache = T, message = F, warning = F, R.options=list(width=120), 
                      fig.width=8, fig.align = 'center', comment=NA, autodep=T, dev='svglite', dev.args = list(bg = 'transparent'))
```

```{r loadpacks, include=FALSE, cache=FALSE}
library(magrittr); library(pander); library(plotly); library(tidyverse); library(lazerhawk); library(broom)
```
