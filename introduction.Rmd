...
# library(reshape2)
mcycle3 = gather(mcycle2, key=variable, value = value, -times, -accel) %>% 
  mutate(variable = factor(variable, labels=c('Polynomial Regression (cubic)', 'Natural Splines', 'Smoothing Splines',
...
```{r mcycleFigure, out.width='50%', echo=FALSE}
mcycleFigure = ggplot(aes(x=times,y=accel), data=mcycle3) + 
  ...
  theme_trueMinimal()
ggplotly(mcycleFigure) %>%    # unlike plot_ly, ggplotly can't handle % width
    theme_plotly()  
```
