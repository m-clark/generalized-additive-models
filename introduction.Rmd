...
maxy = max(y) + 2
plot_ly(width='100%') %>% # marker opacity to 0 to remove idiotic placed endpoints
  add_trace(x=c(minx,minx), y=c(0, maxy), mode='line', showlegend=F,
...
mcycleFigure = ggplot(aes(x=times,y=accel), data=mcycle3) + 
  ...
  theme_trueMinimal()
ggplotly(mcycleFigure, width=500) %>%    # unlike plot_ly, ggplotly can't handle % width
    theme_plotly() %>% 
  layout(paper_bgcolor='rgba(0,0,0,0)',
         plot_bgcolor='rgba(0,0,0,0)')
```
