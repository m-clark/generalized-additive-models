...
Here again we'll use a Markov random field smooth, and for comparison a mixed model with a random effect for district.  The plots shows that, while close, they don't exactly come to the same conclusions for the district fitted values.
...
gam_rent_mrf2 = gam_rent_re
gam_rent_mrf2$smooth = gam_rent_mrf$smooth
# note, coef = fit for re, slight difference with mrf
# cbind(sort(P[[1]]$fit[-336]), sort(coef(gam_rent_mrf)[-1])) %>% plot()
...
As we might have expected, there appears to be more color coordination with the MRF result (left), since neighbors are more likely to be similar.  Meanwhile, the mixed model approach does nothing to correlate one district with the ones it's next to, but may allow for more regularization.
