...
<span class="marginnote">This visualization was created with <span class="pack">plotly</span>. In case you're wondering, it was a notable ordeal to figure out how to make it.</span>

<br>

Using the gaussian process smooth produces a result that is akin to a very traditional spatial modeling technique called <span class="emph">kriging</span>.  There are many other covariates to play with, as well as other bases that would be applicable, so you should feel free to play around with models that include those. 
...
The following will model the percentage of adults with only a high school education.  Unfortunately, when dealing with spatial data,  getting it into a format amenable to modeling, will takes some work. Specifically, <span class="pack">mgcv</span> will need a neighborhood list.  Furthermore, the data we want to use will need to be linked to the data used for mapping.

The first step is to read a shapefile that has some county level information. You could get this from census data as well.


```{r gamspatdiscrete}
# from https://github.com/polygraph-cool/smoothing_tutorial/blob/master/us_county_hs_only.zip
# contiguous states c(1,4:6, 8:13, 16:42, 44:51, 53:56)
library(sp)
shp <- rgdal::readOGR('data/us_county_hs_only', 'us_county_hs_only')
...
The following creates a neighborhood matrix[^usingmaps].  We also need names to match the values in the data, as well as the plotting data to be used later.  I just made them lower case and remove punctuation.  If you use more than one state, you will have to deal with duplicated names in some fashion.
...
With neighborhood in place, we can now finally run the model.  Note that the id used for the smooth, in this case `county`, needs to be a factor variable.  If not you will get an error that doesn't tell you that's that the reason.  There are a couple speed options used, but which would only be required if you're doing many states.
...
Now we can plot it.  <span class="pack">Plotly</span> works with <span class="pack">maps</span> package objects that have been converted via <span class="pack">ggplot2's</span> <span class="func">map_data</span> function.  So we create that and add our fitted values to it. We then add our own coloring based on the fitted values, and a custom clean theme.

```{r plot_mich, out.width='40%', fig.asp=.5}
...
Be prepared, this will be a notable undertaking to sort out for your given situation, depending on the map objects and structure you're dealing with.

#### A Discrete Alternative

One of the things that has puzzled me is just how often people deal with geography while ignoring what would almost always be inherent correlation in discrete geographical or other units.  In the social sciences for example, one will see a standard random effects approach especially, i.e. [a mixed model](https://m-clark.github.io/mixed-models-with-R/), applied in the vast majority of situations where the data comes from multiple regions.  This will allow for region specific effects, which is very useful, but it won't take advantage of the fact that the regions may be highly correlated with one another.  

We've already been using <span class="func">gamm</span>, but haven't been using its typical random effects approach.  We could do so here, but we can also just stick to the usual gam approach, as it has a basis option for random effects.  One thing that distinguishes the mixed model setting is that observations will be clustered within the geographical units.  We'll use the Munich rent data available from the <span class="pack">gamlss</span> family of packages.  It contains the Munich rent data and boundaries files of of 1999 survey. The <span class="objclass">rent99</span> data contains information about rent, year of construction, weather it has central heating, etc. Important for our purposes is the district identifier.
...
The following shows again a Markov random field smooth, and along with it a mixed model with a random effect for district.  The plots shows that, while close, they don't exactly come to the same conclusions for the district fit.
...
# summary(gam_rent_re)
# plot(gam_rent_mrf)
data_frame(mrf_fit = fitted(gam_rent_mrf), mm_fit = fitted(gam_rent_re)) %>% 
  plot_ly() %>% 
  add_markers(x=~mm_fit, y=~mrf_fit, color=I('#ff550040')) %>% 
  theme_plotly()
...
Next we show the plot of the estimated random effects of both models on the map of districts[^munichplot].  Gaps appear because there isn't data for every district available as a polygon, and these are districts without houses like parks, industrial etc.

```{r re_vs_mrf2, echo=FALSE, fig.asp=.4}
# predict(gam_rent_mrf, type = 'terms')
gam_rent_mrf2 = gam_rent_mrf
gam_rent_mrf2$coefficients = gam_rent_re$coefficients

par(mfrow=c(1,2))
plot.gam(gam_rent_mrf)
plot(gam_rent_mrf2, main='Mixed Model', ylab='Mixed Model')
graphics::layout(1)

# fv = fitted(gam_rent_re)
# names(fv) = as.character(rent99$district)
# polys.plot(rent99.polys, fv)
```


As expected there appears to be more color coordination with the MRF result, since neighbors are more likely to be similar.

Either of these models is appropriate, but they ask different questions. The MRF approach may produce better results since it takes into account the potential similarity among neighbors, but also may not be necessary if there isn't much similarity among geographical units.  One should also think about whether the other covariates in the model may account spatial autocorrelation or not, or unspecified unit effects, and proceed accordingly.
...
[^munichplot]: Unfortunately I have neither the data nor the desire to try and make this a pretty plot. It just uses the basic <span class="pack">mgcv</span> plot and a trick to superimpose the mixed model results onto the MRF data.